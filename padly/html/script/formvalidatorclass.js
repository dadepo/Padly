...
FVC.prototype.checkIfExistTest = function(field,valtocheck,db_field)
{

        /*do an ajax call */
        $.get(
                           '/padlytest/padly/html/script/check.php',{tocheck:valtocheck,dtype:'duplicate',db_field:db_field},function(data,status,dxhr)
                           {
                            if (status == 'success')
                            {
                              dup = data;
